...
1. Renaming the controller from `MyPlugin` to `MyPlugin`. In a shell (Git Bash on Windows, replace sed with gsed on macOS):

```bash
sed -i -e's/MyPlugin/MyPlugin/g' `find . -type f`
git mv src/MyPlugin.cpp src/MyPlugin.cpp
git mv src/MyPlugin.h src/MyPlugin.h
git mv etc/MyPlugin.in.yaml etc/MyPlugin.in.yaml
```
